...
export type TaskStore = {
  ...
  findTaskByDedupeKey: <TInput = unknown, TResult = unknown > (
    input: TaskDedupeLookup,
  ) => Promise<TaskRecord<TInput, TResult>|null>;
  claimNextTask: <TInput = unknown, TResult = unknown > (
    input: TaskClaimNextOptions,
  ) => Promise<TaskRecord<TInput, TResult>|null>;
  markTaskRunning: <TInput = unknown, TResult = unknown > (
    input: TaskLeaseInput,
  ) => Promise<TaskRecord<TInput, TResult>|null>;
  renewTaskLease: <TInput = unknown, TResult = unknown > (
    input: TaskLeaseRenewalInput,
  ) => Promise<TaskRecord<TInput, TResult>|null>;
  appendTaskStep: (
    input: TaskAppendStepInput,
  ) => Promise<TaskStepRecord|null>;
  updateTaskProgress: <TInput = unknown, TResult = unknown > (
    input: TaskUpdateProgressInput,
  ) => Promise<TaskRecord<TInput, TResult>|null>;
  markTaskSucceeded: <TInput = unknown, TResult = unknown > (
    input: TaskSuccessInput<TResult>,
  ) => Promise<TaskRecord<TInput, TResult>|null>;
  markTaskFailed: <TInput = unknown, TResult = unknown > (
    input: TaskFailureInput,
  ) => Promise<TaskRecord<TInput, TResult>|null>;
  cancelTask: <TInput = unknown, TResult = unknown > (
    input: TaskCancelInput,
  ) => Promise<TaskRecord<TInput, TResult>|null>;
  markTaskCancelled: <TInput = unknown, TResult = unknown > (
    input: TaskCancelRunningInput,
  ) => Promise<TaskRecord<TInput, TResult>|null>;
  requeueTask: <TInput = unknown, TResult = unknown > (
    input: TaskRetryInput,
  ) => Promise<TaskRecord<TInput, TResult>|null>;
  markStaleTasks: <TInput = unknown, TResult = unknown > (
