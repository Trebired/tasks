...
import { clampPercent, leaseToken, nowIso, sleep, taskId, toErrorShape, withTimeout } from "./utils.js";
    ...
    while (this.#state === "running" && this.#running.size < this.#globalConcurrency) {
      ...
      if (!handler || !task.leaseToken) {
        ...
        await this.#store.markTaskFailed({
          taskId: task.id,
          runnerId: this.#runnerId,
          leaseToken: task.leaseToken || leaseToken(),
          error,
        });
        continue;
      }
      ...
      executionHandle = await this.#executor.execute({
        ...
        onEvent: async (event) => {
          ...
          const step = await this.#store.appendTaskStep({
            ...
            progressPercent: event.step.progressPercent ?? null,
            createdAt: nowIso(),
          });

          if (event.step.progressPercent != null) {
            await this.#store.updateTaskProgress({
              taskId: currentTask.id,
              runnerId: this.#runnerId,
              leaseToken: currentTask.leaseToken || "",
              percent: event.step.progressPercent,
              updatedAt: nowIso(),
            });
          }
